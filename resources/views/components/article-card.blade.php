<article class="group card-shadow overflow-hidden rounded-2xl bg-white transition-all duration-300 hover:-translate-y-1 hover:card-shadow-lg">
    ...
    <div class="p-5">
        <div class="flex items-center gap-3 text-[11px] font-semibold text-ink-soft">
            <span class="flex items-center gap-1"><x-app-icon name="calendar" class="h-3.5 w-3.5 text-forest-600"/>{{ $article->published_label }}</span>
            <span class="flex items-center gap-1"><x-app-icon name="clock" class="h-3.5 w-3.5 text-forest-600"/>{{ $article->read_minutes }} min baca</span>
        </div>
